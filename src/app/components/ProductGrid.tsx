...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  const displayPrice = (() => {
    if (product.variants && product.variants.length > 0) {
      // Get the lowest effective price among all variants
  ...
  return (
    <Link href={`/products/${product._id}`}>
      ...
      >
        ...
        {/* Product Name */}
        <h3 className="font-['Poppins'] font-bold text-[18px] md:text-[20px] text-white mb-0.5 line-clamp-2">
          {product.name}
        </h3>

        {/* Short Description */}
        <p className="font-['Poppins'] font-medium text-[12px] md:text-[13px] text-white/70 mb-3 min-h-[32px] leading-tight line-clamp-2">
        ...
        <div className="flex items-baseline justify-center gap-2 mb-4">
          ...
          {originalPrice && originalPrice > displayPrice && (
            <p className="font-['Poppins'] text-[13px] text-white/40 line-through">
...
export const ProductGrid = () => {
  ...
  if (isLoading) {
    return (
      <section className="bg-black py-10 px-4 lg:px-[139px] overflow-hidden">
        <Reveal width="100%">
          <div className="flex justify-center items-center h-64">
            <div className="text-[#e5e5e5]">Loading products...</div>
          </div>
        </Reveal>
      </section>
    );
  ...
  if (products.length === 0) {
    return (
      <section className="bg-black py-10 px-4 lg:px-[139px] overflow-hidden">
        <Reveal width="100%">
          <div className="flex justify-center items-center h-64">
            <div className="text-[#e5e5e5]">No products available</div>
          </div>
        </Reveal>
      </section>
    );
